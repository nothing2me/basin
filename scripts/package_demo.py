...
if __name__ == "__main__":
    ...
    files = [ROOT / name for name in ["README.md", "LICENSE", "app.py", "requirements.txt", "Start BASIN.cmd", "Setup BASIN.cmd", "start_basin.sh", ".streamlit/config.toml", ".streamlit/credentials.toml"]]
    ...
    if (ROOT / "BASIN.exe").exists():
        files.append(ROOT / "BASIN.exe")
    if (ROOT / "basin.ico").exists():
        files.append(ROOT / "basin.ico")
    if args.wheels:
    ...
    with zipfile.ZipFile(target) as archive:
        ...
        assert not any("/local/" in n or "/.env" in n or "/.venv/" in n for n in archive.namelist())
