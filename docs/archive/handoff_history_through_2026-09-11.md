# BASIN handoff archive — through 2026-09-11

Archived by the September 11 status-reconciliation pass (`docs/status_reconciliation_handoff.md`).
This file is a verbatim copy of `HANDOFF.md` as it stood before that pass, kept so no
evidence, verified test counts, or dated checkpoint narrative is lost. `HANDOFF.md` now
carries only the current-status summary and links back here for history. Nothing in this
archived text was edited — read it as a historical record, not current status; where it
conflicts with `HANDOFF.md`'s current-status section or `TODO.md`, the current documents win.
