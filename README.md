...
The optional **Reservoir simulation** view is an uncalibrated, illustrative experiment. All material assumptions are shown. Review can compare configurable rainfall-retention tiers and assumed conservation cases using a selected storage-system preset; this Review configuration is retained for reports during the session. Assistant-created Region N experiments can be saved with review rationale, inputs and trajectories in schema 2.2 packets and replayed for numerical consistency. These two paths do not yet share a selectable-system persistence contract. Replay does not validate physical assumptions. Neither path predicts reservoir levels, deliveries, safe yield or official restriction dates.
