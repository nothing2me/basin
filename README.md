...
The optional **Reservoir simulation** view is an uncalibrated, illustrative experiment. All material assumptions are shown. It provides a **1-Click Multi-Tier Stress Spectrum** across four rainfall retention tiers (100%, 80%, 60%, 40%) to display illustrative storage-band crossing times and matched conservation comparisons under configured assumptions. If you review and save a simulation, it is versioned to the exact scenario revision and included in the export bundle and brief under its own declared, non-calibration verification scope (schema 2.2); an active stale or unreviewed saved experiment blocks export until resolved; without a saved experiment there is no saved simulation to replay. It does not predict reservoir levels, deliveries, safe yield or official restriction dates.
