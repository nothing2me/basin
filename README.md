...
- **Illustrative stress spectrum:** Sweep configurable rainfall-retention tiers in a selected storage-system experiment and compare assumed conservation cases. Results are replayable numerical experiments, not calibrated forecasts or official trigger dates.
...
**Optional local AI (embedded Qwen).** BASIN works fully offline without it. Local inference needs two separate hash-pinned pieces: the native runtime (the `llama-cpp-python` CPU wheel for Windows x64 / Python 3.12, pinned in `requirements-native.txt`, about 7 MB) and the model weights (about 2.1 GB, downloaded by `scripts/fetch_model.py`). Weights alone do not enable the assistant. Setup asks about each piece separately; for unattended runs use `Setup BASIN.cmd --no-ai`, `--ai-runtime` (runtime only, no weights download), `--with-ai` (runtime and weights) or `--repair-ai`. `.venv/Scripts/python.exe scripts/install_native_runtime.py --check` reports runtime, weights and readiness separately and never downloads anything. The pinned wheel was built with AVX2 enabled and needs the Microsoft Visual C++ 2015-2022 Redistributable (x64); a processor without AVX2 may be unable to load it. See [docs/native_runtime_handoff.md](docs/native_runtime_handoff.md).
...
2. **Workspace:** answer three short questions to tailor the Review layout, then generate complete historical rainfall windows with declared retention factors. Inspect groups and score contributions, compare two or three candidates, and preview alternative priorities on the same pool. The display profile does not change calculations or ranking. Ranking changes preserve your shortlist until an explicit rebuild or swap.
...
The optional **Reservoir simulation** view is an uncalibrated, illustrative experiment. All material assumptions are shown. Review and assistant tools use the workspace's same selected preset or custom storage system. Review previews do not create workflow state; recording an experiment review saves its exact system inputs, settings and trajectories. Saved runs persist in schema 2.2 sessions, feed reports, and replay from the verified ZIP for numerical consistency. Replay does not validate physical assumptions. The experiment does not predict reservoir levels, deliveries, safe yield or official restriction dates.

## Data, limits and privacy

The bundled NOAA GHCN-Daily snapshot covers 1991–2025 with 20 point stations across all 11 Region N counties. A local city/provider run filters choices to its selected counties and preselects the strongest place-name and coverage match; a source-area run keeps every loaded Region N station available for deliberate selection. Alice, Choke Canyon Dam and Corpus Christi form the reproducible regional default. These stations are screening evidence, not validated catchment rainfall. Whole-window sampling preserves complete simultaneous observations, and unavailable station/month/duration combinations stay unavailable rather than being imputed. Retention scales rainfall, not streamflow or hydrologic drought severity. The rainfall reference uses matched onset/duration/stations and windows ending by 2015, with 1991–2020 climatology. It is not the hydrologic drought of record or an occurrence probability.
